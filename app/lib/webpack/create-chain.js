...
module.exports = function (cfg, configName) {
  ...
  if (cfg.ctx.prod || cfg.ctx.mode.ssr) {
    chain.output
      ...
      .filename(cfg.ctx.mode.proton && !cfg.proton.embeddedServer.active ? `js/app.js` : `js/[name]${fileHash}.js`)
  ...
  chain.module.rule('images')
    ...
    .use('url-loader')
      .loader('url-loader')
      .options({
        limit: cfg.ctx.mode.proton && !cfg.proton.embeddedServer.active ? undefined : 10000,
    ...
    if (cfg.build.extractCSS) {
      ...
      chain.plugin('mini-css-extract')
        .use(MiniCssExtractPlugin, [{
          filename: cfg.ctx.mode.proton && !cfg.proton.embeddedServer.active ? 'css/app.css' : 'css/[name].[contenthash:8].css'
        }])

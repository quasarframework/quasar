...
pub fn relative_command(command: String) -> Result<String, std::io::Error> {
    match std::env::current_exe()?.parent() {
        ...
        None => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Could not evaluate executable dir".to_string()))
    }
}
...
pub fn spawn_relative_command(command: String, args: Vec<String>, stdout: Stdio) -> Result<Child, std::io::Error> {

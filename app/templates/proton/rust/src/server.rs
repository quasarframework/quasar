...
fn file_response(path: String) -> HttpResponse {
    ...
    if path.ends_with(".svg") {
        response = response_builder.header("content-type", "image/svg+xml").body(String::from_utf8(asset).unwrap());
    }
    else if path.ends_with(".css") {
        response = response_builder.header("content-type", "text/css").body(String::from_utf8(asset).unwrap());
    }
    else if path.ends_with(".html") {
        response = response_builder.header("content-type", "text/html").body(String::from_utf8(asset).unwrap());
    }
    else {

...
<xtemplate>
  ...
  <!--
      Target Only
  -->
  <div v-if="structure === 'TARGET_ONLY'" class='field field-target' :class='css_Field' ref="ref_FieldTarget">
  ...
  <!--
      Target and Content
  -->
  <div v-else-if="structure === 'TARGET_AND_CONTENT'" class='field field-target' :class='css_Field' ref="ref_FieldTarget">
    <label v-if='txt_Float' class='field-label' :class='css_Float' :for='inputId'>{{ txt_Float }}</label><!-- DO NOT REMOVE!! --><slot></slot>
    <div class='field-swoosh'></div>
    <span v-if='draw_Counter' class="field-counter">{{ txt_Counter }}</span>
    <span v-if='txt_Hint' class="field-hint"><pre>{{ css_Field }}</pre></span>
  ...
  <!--
      Target and User Content
  -->
  <div v-else-if="structure === 'TARGET_AND_USER_CONTENT'" class='field field-target' :class='css_Field' ref="ref_FieldTarget">
    <label v-if='txt_Float' class='field-label' :class='css_Float' :for='inputId'>{{ txt_Float }}</label><!-- DO NOT REMOVE!! --><slot></slot>
    <div class='field-swoosh'></div>
    <span v-if='draw_Counter' class="field-counter">{{ txt_Counter }}</span>
    <span v-if='txt_Hint' class="field-hint"><pre>{{ css_Field }}</pre></span>
...
let STRUCTURE = {
    ...
    TARGET_AND_CONTENT: 'TARGET_AND_CONTENT',
    TARGET_AND_USER_CONTENT: 'TARGET_AND_USER_CONTENT',
    ITEM: 'ITEM'
  }
let TARGET_METHOD = {
    PROP: 'PROP', // TODO: input element provided by props
    CHILD: 'CHILD', // field wraps a single input element
    REF: 'REF', // field wraps multiple elements, target elm specified by "ref='target-input'"
  }
  //  <div v-if="structure === 'TARGET_ONLY'" class='field field-target' :class='css_Field' ref="ref_FieldTarget">
  //   <label v-if='txt_Float' :class='css_Float' :for='inputId'>{{ txt_Float }}</label><!-- DO NOT REMOVE!! -->
  //   <slot></slot>
  //   <div class='field-swoosh'></div>
  //   <span v-if='draw_Counter' class="field-counter">{{ txt_Counter }}</span>
  //   <span v-if='txt_Hint' class="field-hint"><pre>{{ css_Field }}</pre></span>
...
export default {
  render (createElement) {
    ...
    // Prepare Target Children
    //
    if (this.txt_Float) {
      targetChildren.push(
        createElement('label', {
          class: this.css_Float,
          domProps: {
            innerHTML: this.txt_Float,
            for: this.inputId
          }
        }))
    }

    targetChildren.push(this.vnode_input)

    targetChildren.push(
      createElement('div', {
        domProps: {
          className: 'field-swoosh'
        }
      }))
      ...
      targetChildren.push(
        createElement('span', {
          domProps: {
            ...
            innerHTML: this.txt_Hint
          }
        }))
    }
    if (this.draw_Validate) {
      targetChildren.push(
        createElement('span', {
          domProps: {
            className: 'field-validate-msg',
            innerHTML: this.txt_ValidateMsg
          }
        }))
    }
    ...
    //
    let inlineLabel = this.txt_Label ? createElement('label', {
          class: this.css_Float,
          domProps: {
            innerHTML: this.txt_Label
          }
        }) : null

    let icon = this.icon ? createElement('i', {
          class: this.css_Icon,
          domProps: {
            innerHTML: this.icon
          }
    ...
    let icon2 = this.icon2 ? createElement('i', {
          class: this.css_Icon2,
          domProps: {
            innerHTML: this.icon2
          }
        }) : null
    ...
    if (!this.item) {
      // Clean item
      if (icon) fieldChildren.push(icon)
      if (inlineLabel) fieldChildren.push(inlineLabel)
      fieldChildren.push(target)
      if (icon2) fieldChildren.push(icon2)

    } else {
      // Item
      if (icon) fieldChildren.push(icon)
      let contentChildren = []
      if (inlineLabel) contentChildren.push(inlineLabel)
      contentChildren.push(target)
      let content = createElement('div', {
          class: this.css_FieldContent
        },
        contentChildren
      )
      fieldChildren.push(content)
      if (icon2) fieldChildren.push(icon2)
    }

    let field = createElement('div', {
            class: this.css_Field
          },
          fieldChildren
        )
  ...
  computed: {
    ...
    // INIT
    structure () {
      return (!this.item && !this.icon && !this.icon2 && !this.inline) ? STRUCTURE.TARGET_ONLY : STRUCTURE.ITEM
    },
    ...
    css_Field () {
      let
        ...
        css = {
          ...
          [this.item ? 'item multiple-lines' : 'clean-field']: true,
    ...
    css_InlineLabel () {
      return this.item ? 'field-label item-label' : 'field-label'
    },
    css_Icon () {
      return this.item ? 'icon item-primary' : 'icon icon1'
    },
    css_Icon2 () {
      return this.item ? 'icon item-secondary' : 'icon icon2'
    },
    css_Float () {
      // Explanation:
      ...
      //
      return [this.float ? 'field-float field-label' : 'field-float-label field-label']
    },
    style_Field () {
  ...
  beforeMount () {
    ...
    console.log(this, 'Candidate inputs...')
    // let slotElemebts = this.$slots.default.
    let inputs = this.$slots.default.filter(vnode => {
      console.log(vnode)
      return vnode.tag === 'textarea' || vnode.tag === 'input' && TEXT_INPUT_TYPES.includes(vnode.data.attrs.type )
    })
    console.log('Filtered inputs:', inputs)

    if (inputs.length == 0) {
      this.isSingleInput = false
      console.warn('<q-field> missing target', this)
      return
    } if (inputs.length == 1) {
      this.isSingleInput = true
      this.$slots.default=[inputs[0]]
      this.vnode_input = inputs[0]
    } else {
      // TODO: identify input among content. For now, default to first...
      this.isSingleInput = true
      this.$slots.default=[inputs[0]]
      this.vnode_input = inputs[0]
    }

    console.log('beforeMount, input > ', this.vnode_input)
  },
  mounted () {
    console.log(this.float)
    console.log('mounted, candidate inputsEls > ', this.$refs.ref_FieldTarget.childNodes)

    this.input = this.$refs.ref_FieldTarget.querySelector('input, textarea')

    console.log('chose > ', this.input)
    if (!this.input) {
      console.warn('<q-field> missing target <input> or <textarea>.', this)
      // return
    } else if (TEXT_INPUT_TYPES.includes(''+this.input.type)) {
      ...
      console.warn('<q-field> single target must be <input> or <textarea>. Found: ' + this.input.type, this)
      // return
    }
    ...
    if (this.layout === 'inline') {
      ...
      this.myLayout = this.float ? this.floatLayout : 'none'

...
GLOBAL `quasar` CLI; inside a project it defers to the locally installed
...
- `pnpm test:e2e` — boots real `quasar serve` servers and scaffolds a real
  project via `create-quasar` (pnpm install + registry access) to test local
  CLI deferral and `upgrade`. CI: `.github/workflows/cli-tests.yml`.

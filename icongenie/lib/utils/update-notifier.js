...
const checkFlag = '--quasar-update-notifier-check'
...
export function notifyUpdate({ name, version }) {
  ...
  if (now - cache.checkedAt < oneDay) return

  spawn(
    process.execPath,
    [fileURLToPath(moduleUrl), checkFlag, cacheFile, name, version],
    { detached: true, stdio: 'ignore' }
  ).unref()
}

if (process.argv[2] === checkFlag) {

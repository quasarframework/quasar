...
const docsDir = normalize(join(import.meta.dirname, '../..'))
...
/**
 * Boots `quasar dev -m ssr` for the docs site and resolves when it
 ...
 */
export async function startSsrDevServer(port, timeout = 300_000) {
  const child = spawn(
    'pnpm',
    ['exec', 'quasar', 'dev', '-m', 'ssr', '-p', String(port)],
    {
      ...
      detached: true,
      // the docs' devServer.open would pop a browser per run
      env: { ...process.env, QUASAR_DOCS_NO_OPEN: '1' }
    }
  )

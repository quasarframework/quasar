<template>
  <div class="q-pa-md q-gutter-md">
    ...
    <div v-if="pickedFiles.length !== 0" class="q-gutter-sm">
      <q-badge :label="`${pickedFiles.length} file(s)`" />
      <div
        v-for="file in pickedFiles.slice(0, 10)"
        :key="file.webkitRelativePath || file.name"
      >
        {{ file.webkitRelativePath || file.name }}
      </div>
      <div v-if="pickedFiles.length > 10">...</div>
    </div>
  </div>
</template>

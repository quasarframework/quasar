<template>
  <div class="q-pa-md">
    ...
    <div v-if="firstLoaded" class="row justify-between q-mt-md">
      <q-pagination
      ...
      <div class="row items-center">
        <span class="q-mr-sm">Records to page:</span>
        <q-select
          :model-value="pagination.rowsPerPage"
          :display-value="displayRowsPerPage"
...
export default {
  setup () {
    ...
    // options select for rows per page
    const mapOptions = [ 5, 7, 10, 15, 20, 25, 50, 0 ].map(count => ({
      label: count === 0 ? 'All' : '' + count,
      value: count
    }))

    const rowsPerPageOptions = ref(mapOptions)

    const displayRowsPerPage = computed(
      () => pagination.value.rowsPerPage === 0 ? 'All' : pagination.value.rowsPerPage
    )
    ...
    function onRequest (props) {
      ...
      setTimeout(() => {
        ...
        // enable first loading indicator

...
This guide is for when you want to ensure that a <llm-exclude when="mcp" reason="the components index is an interactive page of the site">[Quasar Plugin](/components)</llm-exclude><llm-only when="mcp" reason="the same words without the link to the site-only index">Quasar Plugin</llm-only> will be injected into the hosting app, because you depend on it for your own App Extension to work.

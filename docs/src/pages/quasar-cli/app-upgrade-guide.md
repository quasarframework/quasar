...
* Dependencies
  ...
  - Use "quasar upgrade -i" command (recommended) or manually yarn/npm install @quasar/app v2: `yarn add --dev @quasar/app@^2.0.0` (or `npm install --save-dev @quasar/app@^2.0.0`). Either way, confirm if its successfully installed with `quasar --version` or look inside `package.json` file where `@quasar/app` is set to `^2.0.0`.

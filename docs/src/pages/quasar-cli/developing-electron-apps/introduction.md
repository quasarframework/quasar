...
Comparing with client-server architecture, main thread can be considered as server and renderer thread(s) as client(s) beacuse all client(s) (renderer thread(s)) are initiated by and communicate with server(main thread) and each client (renderer thread) executes in isolation and independence of other client(s) (renderer thread(s)). However the server (main thread) has access to all client(s) (renderer thread(s)).

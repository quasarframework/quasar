...
Since Quasar is based on Vue, you will want to refer to Vue's cookbook for setting up debugging Vue apps using VSCode. You can find that here:
- [Vue Cookbook for VSCode debugging](https://vuejs.org/v2/cookbook/debugging-in-vscode.html)
...
The first step the cookbook says is to enable source maps. Quasar automatically enables source maps for development mode. Here is [a good article](https://blog.scottlogic.com/2017/11/01/webpack-source-map-options-quick-guide.html) that describes the different values for the [webpack devtool setting](https://webpack.js.org/configuration/devtool/) (the one that turns on or off source maps.) Quasar uses _cheap-module-eval-source-map_. 

I have found that cheap-module-eval-source-map may build faster, but it makes debugging harder and it make debugging in VSCode near impossible. The recommended value of devtool for debugging within VSCode is _source-map_. This makes debugging in VSCode work properly. It also makes your full vue source file available in the built in chrome debugger, making it easier to find your original source and easier to correctly locate the line you want to set the break point on. If you want to enable this, you would add this line to your _quasar.config.js_ file. 
...
```
This will set this value for devtool only in the builds where source maps are enabled. By default the dev (development build) command enables source maps and the build (production build) command disables source maps. 

Then you need to configure VSCode by adding a configuration to the debugger. The easiest way to do that is to click on the bug icon on the action bar (for ltr languages, that is the bar on the far left). Once you click on that bug icon, the file tree area will switch to the debug and run area. Click on the gear icon in the title bar of that window and it will bring up a file called launch.json. This is where you put the different configurations of launching the application to be debugged. Here are the settings for launching a Quasar app in chrome. For the firefox version, look at the vue cookbook mentioned above. 
...
Now save the file, then select that configuration in the drop down in the title bar of the debug and run pane. Before you can launch the debugger, the app must be running. From the command line, launch dev mode of your app with:
`quasar dev`
Then click the green go button in the debug and run pane to launch the debugging session and attach to your running app. You can now set break points in VSCode and control step over/in/out etc, all from VSCode. You can also launch the built in chrome debugger and they will stay in sync. This might be useful if you have the [Vue tools](https://chrome.google.com/webstore/detail/vuejs-devtools/nhdogjmejiglipccpnnnanhbledajbpd) of the chrome debugger installed (highly recommended).

::: tip
If you just want to use the chrome / firefox debugger, but you find it hard to find the right source file in the chrome source browser, you can use the debugger statement in your code to force the debugger to stop on that line and bring up the proper source code in its built in browser.
:::

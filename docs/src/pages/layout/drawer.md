...
* If you use a QScrollArea with some fixed elements on top and/or bottom as the content of a QDrawer wrap them in a `<div class="fit overflow-hidden column no-wrap">` and use a `col` class on the QScrollArea to get automatic height for the QScrollArea
:::

...
>
> The composable can also be called outside of `setup()`: in a boot file, a store or a plain module. There is no mount to wait for there, so the stream opens right away (unless `manualOpen` is set) and nothing closes it by itself: call `closeSource()` when you are done.
...
setup () {
  const { sourceStatus, data, lastEventId, error, openSource, closeSource } = useEventSource(
    url, // String, URL, or a ref/getter of one
    {
      ...
      manualOpen: true, // do not open the stream on mount;
                        // openSource() does it
...
The stream opens when the component is mounted (or right away, when the composable is used outside of a component) and is closed when the component gets destroyed. Set `manualOpen: true` for a stream that should wait for your `openSource()` call.
...
`data` holds the `data` of the last event received (always a String: parse it yourself with `JSON.parse()` if your server sends JSON) and `lastEventId` the `id` of the last event that carried one. `error` holds the last `error` event of the stream. The `onMessage`, `onError`, `onOpen` and `onClose` hooks get called in the same situations, so you do not need to watch the refs.
...
The browser reconnects by itself when a stream's connection drops (respecting the `retry:` field sent by the server) and fires an `error` event before each try; the composable reports it through `error` and `onError` while `sourceStatus` says `connecting`, and nothing else is needed.
...
A stream reopened by the composable is a new `EventSource`, so the browser does not send the `Last-Event-ID` header the way it does on its own retries. If your server can resume from an id, pass `lastEventId` along in the URL yourself when you call `openSource()` after a close.

## Example

The example below listens to Wikimedia's public stream of recent edits, which pushes several events a second. The stream waits for your click (`manualOpen`); open and close it to see the status follow.

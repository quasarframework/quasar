...
export function setPosition ({ el, anchorEl, anchorOrigin, selfOrigin, offset, absoluteOffset, cover, fit }) {
  ...
  const
    ...
    { innerHeight: fullHeight, innerWidth: fullWidth } = window
  ...
  if (props.top >= anchorProps.top) {
    ...
    el.style.top = 'auto'
    el.style.bottom = Math.max(0, fullHeight - props.top - offset) + 'px'
  }
  ...
  if (props.left >= anchorProps.left) {
    ...
    el.style.left = 'auto'
    el.style.right = Math.max(0, fullWidth - props.left - offset) + 'px'
  }

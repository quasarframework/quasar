...
export default Vue.extend({
  ...
  methods: {
    ...
    __getFirstSelectedDate (val, rangeStart = false) {
    ...
    __getLastSelectedDate (val, rangeStart = false) {
      if (Array.isArray(val)) {
        let last = val.slice().pop()
        return Array.isArray(last) ? last[1 - rangeStart] : last
      }
      else {
        return val
      }
    ...
    __getRangeStart (val) {
      return this.__getLastSelectedDate(val, true)
    },
      ...
      if (Array.isArray(val)) {
        ...
        val.forEach((value, index) => {
          if (Array.isArray(value)) {
            array[index] = [
              extractDate(value[0], mask, locale),
              extractDate(value[1], mask, locale)
            ]
          }
          else { 
                ...
                day.in === true
                  ? h(QBtn, {
                    ...
                    on: cache(this, 'day#' + day.i, { 
                      click: event => {
                        if (this.range && event.shiftKey) {
                          this.__setRangeEndDay(day.i)
                        }
                        else if (this.multiple && (event.ctrlKey || event.shiftKey)) {
    ...
    __updateValue (date, reason) {
      ...
      if (['set-range-end-day', 'add-remove-day'].includes(reason)) {

    ...
    <div class="q-layout-padding text-center" id="view-colors">
      ...
      >
        ...
        <q-btn
          ...
          size="0.65rem"
          icon="undo"
          @click="undoColor(color)"
          v-if="
            color !== 'black' &&
            mainColorValues[color] !== mainColorValuesOrig[color]
          "
        />
      </div>
      <div
      ...
      <div v-if="currentColor" class="row justify-center items-end q-mt-md">
        <q-color
          :model-value="mainColorValues[currentColor]"
          @update:model-value="val => setColor(currentColor, val)"
        />
      </div>
      ...
      <div class="detail" v-for="color in colors" :key="color">
        <h5
          class="detailed-color shadow-1 column flex-center text-white"
          :class="'bg-' + color"
        >
          {{ color }}
        </h5>

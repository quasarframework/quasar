  ...
  <div class="q-layout-padding q-gutter-md">
    <q-card flat bordered>
      ...
      <q-card-section class="row q-gutter-sm">
        ...
        <q-btn flat label="stopDropZone()" @click="stopDropZone" />
      </q-card-section>
      <q-separator />
      <q-card-section>
        <div class="q-gutter-sm">
          <q-badge :label="`isOverDropZone: ${isOverDropZone}`" />
          <q-badge :label="`dropped: ${acceptedDropZoneFiles.length}`" />
          <q-badge
            color="negative"
            :label="`rejected: ${rejectedDropZoneFiles.length}`"
          />
        </div>
        <q-list dense v-if="acceptedDropZoneFiles.length !== 0" class="q-mt-sm">
          <q-item v-for="(file, index) in acceptedDropZoneFiles" :key="index">
        ...
        <q-list
          dense
          v-if="rejectedDropZoneFiles.length !== 0"
          class="q-mt-sm text-negative"
        >
          <q-item v-for="(entry, index) in rejectedDropZoneFiles" :key="index">

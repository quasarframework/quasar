    ...
    <div class="q-gutter-y-md">
      <div class="row q-col-gutter-md justify-stretch">
        ...
        <div v-for="n in 5" :key="'r-link.' + n">
          <router-link
            class="special-router-link"
            replace
            :to="{ query: { tab: '' + n, noScroll: true } }"
            active-class="special-router-link--active"
            exact-active-class="special-router-link--exact-active"
          >
            [ {{ n }} ]
          </router-link>
        </div>
        ...
        <div>
          <q-item
            class="special-router-link"
            replace
            :to="{ query: { tab: '3', noScroll: true, y: '5' } }"
            active-class="special-router-link--active"
            exact-active-class="special-router-link--exact-active"
          >
      ...
      <q-tabs no-caps class="bg-orange text-white">
        <q-route-tab
          replace
          :to="{ query: { tab: '1', noScroll: true } }"
          label="[1*] Activate in 2s"
          @click="navDelay"
        />
        <q-route-tab
          replace
        ...
        <q-route-tab
          replace
          :to="{ query: { tab: '3', noScroll: true } }"
          label="[3*] Navigate to the second tab"
          @click="navRedirect"
        />
        <q-route-tab
          replace
        ...
        <q-route-tab
          replace
          :to="{ query: { tab: '4', noScroll: true } }"
          label="[4*] Navigate immediately"
          @click="navPass"
        />

        <q-route-tab
          replace
          :to="{ query: { tab: '5', noScroll: true } }"
          label="[5*] With button"
          @click="navPass"
        >
          <q-btn

...
export default {
  ...
  methods: {
    runTest (fixture, runFn) {
      const index = this.testList.length
      ...
      this.testList.push({
        ...
        copy: () => {
          copyToClipboard(this.testList[ index ].result)
            .then(() => {
              Notify.create({
                type: 'positive',
  ...
  created () {
    ...
    this.runTest([ 'list', 'resolve all', 'yes', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(), { abortOnFail: true }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
    ...
    this.runTest([ 'list', 'resolve all', '-', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(), { abortOnFail: false }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
            { key: 0, status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'list', 'resolve all', 'yes', '2 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(), { abortOnFail: true, threadsNumber: 2 }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
            { key: 0, status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'list', 'resolve all', '-', '2 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(), { abortOnFail: false, threadsNumber: 2 }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
            { key: 0, status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'list', 'reject 2nd', 'yes', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(true), { abortOnFail: true }).then(list => {
    ...
    this.runTest([ 'list', 'reject 2nd', '-', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseList(true), { abortOnFail: false }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
            { key: 0, status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'list', 'reject 2nd', 'yes', '2 threads' ], setResult => {
      runSequentialPromises(this.getPromiseList(true), { abortOnFail: true, threadsNumber: 2 }).then(list => {
        setResult(list, false)
      }).catch(result => {
        setResult(
          result,
    ...
    this.runTest([ 'list', 'reject 2nd', '-', '2 threads' ], setResult => {
      runSequentialPromises(this.getPromiseList(true), { abortOnFail: false, threadsNumber: 2 }).then(list => {
        setResult(
          list,
          is.deepEqual(list, [
            { key: 0, status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'map', 'resolve all', 'yes', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(), { abortOnFail: true }).then(resultAggregator => {
        setResult(
          resultAggregator,
    ...
    this.runTest([ 'map', 'resolve all', '-', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(), { abortOnFail: false }).then(resultAggregator => {
        setResult(
          resultAggregator,
          is.deepEqual(resultAggregator, {
            one: { key: 'one', status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'map', 'resolve all', 'yes', '2 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(), { abortOnFail: true, threadsNumber: 2 }).then(resultAggregator => {
        setResult(
          resultAggregator,
          is.deepEqual(resultAggregator, {
            one: { key: 'one', status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'map', 'resolve all', '-', '2 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(), { abortOnFail: false, threadsNumber: 2 }).then(resultAggregator => {
        setResult(
          resultAggregator,
          is.deepEqual(resultAggregator, {
            one: { key: 'one', status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'map', 'reject 2nd', 'yes', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(true), { abortOnFail: true }).then(resultAggregator => {
    ...
    this.runTest([ 'map', 'reject 2nd', '-', '1 thread' ], setResult => {
      runSequentialPromises(this.getPromiseMap(true), { abortOnFail: false }).then(resultAggregator => {
        setResult(
          resultAggregator,
          is.deepEqual(resultAggregator, {
            one: { key: 'one', status: 'fulfilled', value: 'one' },
    ...
    this.runTest([ 'map', 'reject 2nd', 'yes', '2 threads' ], setResult => {
      runSequentialPromises(this.getPromiseMap(true), { abortOnFail: true, threadsNumber: 2 }).then(resultAggregator => {
        setResult(resultAggregator, false)
      }).catch(result => {
        setResult(
          result,
    ...
    this.runTest([ 'map', 'reject 2nd', '-', '2 threads' ], setResult => {
      runSequentialPromises(this.getPromiseMap(true), { abortOnFail: false, threadsNumber: 2 }).then(resultAggregator => {
        setResult(
          resultAggregator,
          is.deepEqual(resultAggregator, {
            one: { key: 'one', status: 'fulfilled', value: 'one' },
